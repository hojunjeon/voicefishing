...
> **기본 LLM Provider:** 로컬 Codex CLI ChatGPT OAuth (`gpt-5.5`, `low`)<br>
...
|---:|---|---|
| 1 | 전체 시스템 아키텍처 | Next.js·FastAPI·Codex ChatGPT OAuth·Voice/TTS·PostgreSQL의 책임과 경계 |
...
Aegis MVP는 **Next.js 웹 클라이언트 + FastAPI 모듈형 모놀리스 + PostgreSQL/pgvector + 로컬 Codex ChatGPT OAuth + Voice Clone/TTS Provider**로 구현한다.
...
| LLM Provider | 로컬 Codex ChatGPT OAuth | Scenario Judge·Responder·Extractor·Scammer Simulator를 단일 Adapter로 연결 |
| 공개 배포 경계 | OAuth 로그인·LLM 실행 차단 | 사용자 로컬 자격증명을 Vercel에 복사하지 않음 |
...
| AI Provider | Codex OAuth Adapter | Scenario Judge, Responder, Extractor, Scammer Simulator |
...
MVP의 `LLMProvider` 기본 구현은 로컬 Codex CLI의 ChatGPT OAuth 세션을 재사용한다. 모델은 `gpt-5.5`, 추론 수준은 `low`로 고정하며 `auth.json`을 직접 읽거나 공개 런타임에 업로드하지 않는다.
...
- 로컬 데모 모델은 `gpt-5.5`, 추론 수준은 `low`로 고정

```text
LLM_PROVIDER=codex_chatgpt_oauth
MODEL=gpt-5.5
...
| LLM | Codex ChatGPT OAuth Adapter | 네 역할의 공통 모델 실행점 |
...
| **C. AI / RAG / Voice / Eval** | Codex OAuth·RAG·Safety·음성 | Scenario Judge, Scenario RAG, Safety, Adversarial RAG, Responder, Extractor, Simulator, Voice Adapter, 평가셋 |
...
| 2 | 프로젝트 골격 | Next.js route와 공통 store | FastAPI, migration, session API | Fake Provider, Codex OAuth Adapter 골격 |
...
| Contract | Codex 구조화 출력, Extractor JSON, WS payload, Provider Adapter |
...
| Codex ChatGPT OAuth `gpt-5.5/low` | 2장, 9장 | 충족 |
...
| 위험 | 영향 | 대응 |
|---|---|---|
| Codex 구조화 출력 스키마 차이 | Extractor/State 오류 | Adapter + JSON Schema + fixture |
